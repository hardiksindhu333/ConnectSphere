    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {isEditing ? (
          ...
          <div className="mt-1 text-gray-200 whitespace-pre-wrap break-words">{comment?.content}</div>
        )}
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <IconButton onClick={() => setIsReplying((v) => !v)} title="Reply">
...
function ReplyItem({ reply, currentUserId, onReply, onEdit, onDelete, onLike, isLiking, isDeleting }) {
  const [isEditing, setIsEditing] = useState(false);
  const [editText, setEditText] = useState(reply?.content || "");
  const [isReplying, setIsReplying] = useState(false);
  const avatar = resolveMediaUrl(reply?.owner?.avatar?.url || reply?.owner?.avatar);
  const mine = reply?.owner?._id && currentUserId && reply.owner._id === currentUserId;

  return (
    <div className="flex gap-3">
      {avatar ? (
        <img src={avatar} className="w-8 h-8 rounded-full object-cover bg-white/10" />
      ) : (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <div className="text-sm font-semibold">{reply?.owner?.username}</div>
        ...
        {isEditing ? (
          ...
          <div className="mt-1 text-gray-200 whitespace-pre-wrap break-words">{reply?.content}</div>
        )}

        <div className="mt-2 flex flex-wrap items-center gap-2">
          <IconButton onClick={() => onLike?.(reply._id)} disabled={isLiking} title="Like">
            <Heart size={16} />
            <span className="text-sm text-gray-300">{reply?.likesCount || 0}</span>
          </IconButton>
        ...
        {reply?.replies?.length ? (
          <div className="mt-3 pl-4 border-l border-white/5 space-y-4">
